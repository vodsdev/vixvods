...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><div className="eyebrow">Profil entreprise · {company.location}</div><Reveal><section className="hero company-hero" style={{paddingBottom: "40px"}}><div><h1 style={{fontSize: "clamp(42px, 6vw, 72px)"}}>{company.name}</h1><p className="lede">{company.description}</p><a href={company.website} className="text-link" target="_blank" rel="noreferrer">Visiter le site →</a></div><div className="hero-card"><div className="eyebrow">Trust Score indicatif</div><div className="score" style={{fontSize: "72px", margin: "18px 0 0"}}>{trust}<span style={{fontSize: "26px"}}>/100</span></div><div className="score-row"><span className="stars">{"★".repeat(Math.round(average))}{"☆".repeat(5 - Math.round(average))}</span><span className="muted">{reviews.length} avis</span></div></div></section></Reveal><Reveal><section className="section grid"><div><div className="section-head"><h2>Avis publiés</h2></div>{reviews.length === 0 ? <div className="card"><p>Aucun avis publié pour le moment.</p></div> : <div className="review">{reviews.map((review) => <article className="card review" key={review.id}><div className="review-meta"><span><strong>{review.author}</strong> <span className="pill">{review.hasProof ? "Preuve déclarée" : "Avis partagé"}</span></span><span className="muted">{new Date(review.createdAt).toLocaleDateString("fr-FR")}</span></div><div className="stars">{"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}</div><p>{review.comment}</p><div className="notice"><strong>Analyse :</strong> {review.aiSummary}<br /><span className="muted">Thèmes : {review.keywords.join(" · ")}</span></div>{review.aiResponse && <p className="muted"><strong>Réponse :</strong> {review.aiResponse}</p>}</article>)}</div>}</div><ReviewForm companySlug={company.slug} /></section></Reveal></main>;
}
